...
![image-20241014171313705](C:\Users\86188\AppData\Roaming\Typora\typora-user-images\image-20241014171313705.png)

- ebpf_prog用于kmesh关闭后进行流量治理操作
- ebpf_map用于记录kmesh关闭后提供流量治理操作的配置
- inner_map_mng用于记录序列化过程中的一些信息，帮助kmesh重启恢复时能够正确处理ebpf_map之间的关系
- hashName用于记录每棵XDS配置树的hash值，用于比较XDS配置树是否发生变化
- kmesh_version用于记录kmesh的版本信息，用于比较kmesh是否是重启/升级

#### ebpf程序持久化

- ebpf_prog有Sockconn/sockops/tracepoint，使用bpf_link将attach过的Sockconn/sockops的bpf程序固化，tracepoint程序直接pin到目录上

- 固化其他ebpf map，直接pin到目录上
...
将XDS配置树计算出hash并保存于/mnt/hashName目录下，用于重启后数据对比

### 配置恢复与更新

1. 重启后加载ebpf程序

   从pin的指定目录恢复bpf_map，inner_map，kmesh_version

   从指定目录恢复inner_map_mng信息

   启动新的ebpf程序Sockconn/sockops，attach后更新替换掉bpf_link中的prog，完成无缝替换

   tracepoint的ebpf程序启动新的之后，attach上再删除旧的，完成替换
