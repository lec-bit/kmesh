...
func TestGetCniConfigPath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			config := tt.utconfig
			tt.beforeFunc()
			i := NewInstaller(constants.KernelNativeMode, config.CniMountNetEtcDIR, config.CniConfigName, config.CniConfigChained, "")
...
func TestInsertCNIConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tt.beforeFunc()
			i := NewInstaller(constants.KernelNativeMode, "", "", true, "")
