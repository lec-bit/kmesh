...
u32 parse_http_1_1_request(const struct bpf_sock_addr_kern *ctx);

u32 parse_http_1_1_respond(const struct bpf_sock_addr_kern *ctx);
...
static bool parse_header(struct bpf_mem_ptr *context)
{
    ...
    for (i = 0; !head_end && i < context->size; ++i) {
        ...
        case ST_FIELD_NAME_START:
            if (ch == FIELD_SPLIT) {
                LOG("Invalid field split detected, char：%c, current_state:%d\n", ch, current_state);
                return false;
            }
            if (ch == CR) {
        ...
        case ST_NEW_LINE:
            ...
            if (field_name_end_position < field_name_begin_position || 
                ...
                LOG("Invalid field name or value positions, char:%c, current_state:%d\n", ch, current_state);
                return false;
            }
            new_field = new_kmesh_data_node(field_name_end_position - field_name_begin_position + 2);
            if (IS_ERR(new_field)) {
                LOG("Failed to create new field node, char:%c, current_state:%d\n", ch, current_state);
                return false;
            }
            (void)strncpy(
                new_field->keystring,
    ...
    if (current_state != ST_HEAD_END) {
        LOG("parse_header failed, current_state:%d\n", current_state);
        return false;
    }
    return true;
}
...
u32 parse_http_1_1_request(const struct bpf_sock_addr_kern *ctx)
{
    ...
    if (ctx->t_ctx != NULL) {
        ...
        LOG("t_ctx is NULL, parse request failed\n");
        return PROTO_UNKNOW;
    }
...
u32 parse_http_1_1_respond(const struct bpf_sock_addr_kern *ctx)
{
    ...
    if (ctx->t_ctx != NULL) {
        ...
        LOG("t_ctx is NULL, parse respond failed\n");
        return PROTO_UNKNOW;
    }
