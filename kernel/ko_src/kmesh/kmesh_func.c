...
__bpf_kfunc int
bpf_km_header_strnstr_func(void *ctx, int ctx__sz, const char *key, int key__sz, const char *subptr)
{
...
__bpf_kfunc int
bpf_km_header_strncmp_func(const char *key, int key_sz, const char *target, int target_len, int opt)
{
        return bpf_km_header_strncmp_impl(key, key_sz, target, target_len, opt);
}

__bpf_kfunc int
bpf_parse_header_msg_func(void *bpf_mem, int src__sz)
{
...
__bpf_kfunc int
bpf_setsockopt_func(void *bpf_mem, int bpf_socket__sz, int optname, const char *optval, int optval__sz)
{
        struct bpf_mem_ptr *tmp = bpf_mem;
        struct bpf_sock_addr_kern *ctx = tmp->ptr;
...
__bpf_kfunc int
bpf_getsockopt_func(void *bpf_mem, int bpf_socket__sz, int optname, char *opt, int opt__sz)
{
        struct bpf_mem_ptr *tmp = bpf_mem;
        struct bpf_sock_addr_kern *ctx = tmp->ptr;
        struct sock *sk = ctx->sk;
